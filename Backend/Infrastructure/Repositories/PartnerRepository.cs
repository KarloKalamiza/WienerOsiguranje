...
public class PartnerRepository : IPartner
{
    ...
    public async Task<IEnumerable<PartnerResponse>> FindPartners(int numOfPolicies, decimal entirePolicyAmount)
    {
        if (numOfPolicies < 0 || entirePolicyAmount < 0)
            throw new Exception("Cannot be negative numbers.");
        try
        {
            IEnumerable<PartnerResponse> partners = await GetPartnerWithPolicies();
            var filteredPartners = partners
            .Where(p => (p.Policies != null && p.Policies.Count > numOfPolicies) ||
                        (p.Policies != null && p.Policies.Sum(policy => policy.PolicyAmount) > entirePolicyAmount))
            .ToList();
            return filteredPartners;
        }
        catch (Exception ex)
